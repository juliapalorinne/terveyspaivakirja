...
def get_all_workouts(user_id):
    sql = "SELECT id, name, time FROM workouts WHERE user_id=:user_id ORDER BY time DESC"
...
def add_workout(user_id, name, category, comments):
    sql = "INSERT INTO workouts (name, category, comments, time, user) VALUES (:name, :category, :comments, NOW(), :user_id) RETURNING id"
...
def change_workout_info(id, username, name, height, weight):
    sql = "UPDATE users SET (username=:usrename, name=:name, height=:height, weight=:weight) WHERE id=:id"
    db.session.execute(sql, {"username":username, "name":name, "height":height, "weight":weight, "id":id})
    db.session.commit()
